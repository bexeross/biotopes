...
my.subs<-subset(clss.ref.mat,class =="Tmp8") #change as needed
```
...
#parent groups
piv.meta.a<-my.subs %>%
  group_by(level10)%>% # change to level you want to see groups at
...
```{r}
 ...
 write.csv(out.piv.short, 
           file = file.path(outPath, ("Tmp8_2_avGT1_propGT50pc.csv"))) #change label


write.csv(out.piv, 
          file = file.path(outPath, ("Tmp8_2_av_proportion_max.csv"))) #change label

write.csv(piv.meta, 
          file = file.path(outPath, ("Tmp8_2_metadata.csv"))) #change label
