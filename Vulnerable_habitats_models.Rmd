...
crossvalidate <- function(data, k, model, dependent){
  ...
  for (fold in 1:k){
    ...
    # else use lm()
    modelmodel <- cforest(model, training_set, control = cforest_unbiased(ntree=500,mtry = 3))
    ## Test model
...
```{r}
library(spm)
load("U:/Mareano/VIDEOLAB/VIDEO DATA/200m_scale_species_by_sample/Data_Delivery_2021/outputs/WORKSPACE_modeltraining.Rdata")
