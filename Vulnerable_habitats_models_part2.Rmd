...
plot(c(crossvalidate(train_set,4,fmla0,paste0("X",resp)),
crossvalidate(train_set,4,fmla1,paste0("X",resp)),
crossvalidate(train_set,4,fmla2,paste0("X",resp)))~c(0,1,2), xlab = "model", ylab = "(R)MSE")
```
...
save(list=c("train_set", "test_set", "fmlabest", "resp"), file = "WORKSPACE_modeltraining.Rdata") # save in working dir
```
...
crossvalidateVE <- function(data, k, model, dependent){
  require(Metrics)
  # 'data' is the training set with the ".folds" column
  # 'k' is the number of folds we have
  # 'model' is a string describing a linear regression model formula
  ...
  for (fold in 1:k){
    ...
    # use lmer() to train model
    # else use lm()
    modelmodel <- party::cforest(model, training_set, 
                          #weights = training_set$count,
                          control = cforest_unbiased(ntree=500,mtry = 3))
    ## Test model
    # Predict the dependent variable in the testing_set with the trained model
    ...
    o <- testing_set %>% select(contains(resp))
    crosscheck <- data.frame(predicted = unlist(predicted), observed = pull(o))
    ve <- vecv(crosscheck$observed,crosscheck$predicted)
        # Add the RMSE to the performance list
    performances[fold] <- ve
  }
  # Return the mean of the recorded RMSEs
  return(c('VE' = mean(performances)))
}
crossvalidateVE(train_set,4,fmlabest,paste0("X",resp))
```
