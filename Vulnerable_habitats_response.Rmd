...
library(agrmt)
x <- data.frame( species = row.names(indval3), biotope = colnames(indval3)[max.col(indval3,ties.method="first")],
...
respdf <- spe3 %>% mutate("3031" = select(., c("30", "31")) %>% rowSums(na.rm = TRUE)) %>% set_rownames(spe2$SampID)
