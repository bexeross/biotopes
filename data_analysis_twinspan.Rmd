...
Convert from long format input to a wide format where rows are samples and columns are species
```{r}
...
We should use only video lines where data is complete. We determine that by looking at the total number of biological annotations relative to the towed distance. Note that total number of biological annotations depends on species richness!  
```{r}
hist(refer$total_bio/refer$towed_distance)
```
### Make a list of "good" stations
Check how much data we would be using/discarding
```{r}
thres <- 3
...
Decide what is acceptable in terms of length of sample and picture quality
```{r}
...
### Use all samples and all species
```{r}
tw_1 <- trimmedwide %>%
  select(-SampID) %>%
