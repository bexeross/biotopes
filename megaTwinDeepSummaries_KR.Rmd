...
piv.av<-my.subs %>%
  group_by(class)%>% # change to level you want to see groups at
  summarise(across(c(17:147),mean)) %>%
  t()%>%
 # na.omit()%>%
  `colnames<-`(.[1, ])%>%
  .[-1, ] %>%
...
piv.n<-my.subs %>%
  group_by(class)%>% # change to level you want to see groups at
  summarise(across(c(17:147),nnzero)) %>%
  t()%>%
  #na.omit()%>%
  as.data.frame() %>%
  `colnames<-`(.[1, ])%>%
...
This means merging with the exclded stations and retaining only the SampID and the biotope class.

##import (if not done above)
```{r}
new.data<-trimmedwide %>%
  filter(SampID %in% fj.newfilt.out$SampID)
...
writeOGR(sites.class.shp, dsn = file.path(outPath, "Spatial"), "KR_classified", driver = "ESRI Shapefile", overwrite_layer = TRUE)
