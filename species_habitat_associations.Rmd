...
```{r}
# Prepare the data
spe <- trimmedwide %>% ungroup() %>% select(-SampID)%>%mutate_if(is.numeric, ~replace(., is.na(.), 0))
hab <- hab
...
indval <- data.frame(strassoc(X = spe, cluster = hab, func = "IndVal.g")) # Compute point-biserial correlation (group-equalized), ranges 0-1
#indval <- data.frame(strassoc(X = spe, cluster = hab, func = "r.g"))

indval <- indval %>% filter(row.names(.)%in%indsp)

write.csv(round(indval, digits = 2), file.path(dataPath, "outputs\\TMP class 0122\\indicator_value.csv"), row.names = TRUE)
```
